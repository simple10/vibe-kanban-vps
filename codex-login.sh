#!/bin/bash
set -euo pipefail

# =============================================================================
# OpenAI Codex CLI Login Helper
# SSHs into the VPS, docker execs into the vibe-kanban container, and runs
# the Codex CLI login flow so agents can use OAuth instead of an API key.
#
# Usage: bash codex-login.sh
# =============================================================================

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# --- Load .env --------------------------------------------------------------
if [[ -f "${SCRIPT_DIR}/.env" ]]; then
    set -a
    # shellcheck disable=SC1091
    source "${SCRIPT_DIR}/.env"
    set +a
else
    echo "ERROR: .env not found in ${SCRIPT_DIR}" >&2
    exit 1
fi

# --- Validate required vars -------------------------------------------------
: "${VPS_IP:?VPS_IP is required in .env}"
: "${SSH_KEY_PATH:?SSH_KEY_PATH is required in .env}"
SSH_USER="${SSH_USER:-root}"
SSH_PORT="${SSH_PORT:-22}"

SUDO=""
if [[ "${SSH_USER}" != "root" ]]; then
    SUDO="sudo"
fi

SSH_CMD="ssh -t -i ${SSH_KEY_PATH} -p ${SSH_PORT} -o StrictHostKeyChecking=accept-new ${SSH_USER}@${VPS_IP}"

# --- Run Codex CLI login inside the container --------------------------------
echo "Connecting to ${VPS_IP} and running Codex CLI login inside the vibe-kanban container..."
echo "A URL will be displayed — open it in your browser to complete the login."
echo ""

$SSH_CMD "${SUDO} docker exec -it -u vkuser -e HOME=/home/vkuser vibe-kanban npx -y @openai/codex@latest login"
